<?php
  ...
  $hlastnow = @fopen("http://apache2-simple/guifi/refresh/maps", "r") or die('Error reading changes\n');
  ...
  $hlinks = @fopen("http://apache2-simple/guifi/gml/".$rootZone."/links/csv", "r") or die("Error getting links cv\n");;
  if ($hlinks) {
   while (!feof($hlinks)) {
       print_r($member);
...
<ogr:FeatureCollection
  ...
  $hnodes = @fopen("http://apache2-simple/guifi/gml/".$rootZone."/nodes/csv", "r");
  if ($hnodes) {
   while (!feof($hnodes)) {
